...
def grouped_mm_experts_forward(
    ...
    # 1. Slice offsets to match the number of available experts 
    offsets = offsets[:selected_gate_up.size(0)]

    # 2. Slice the input tensor to match the tokens covered by these experts.

    valid_token_count = offsets[-1].item()
    selected_hidden_states_g = selected_hidden_states_g[:valid_token_count]

    # 3. Slice the bias to match the valid tokens
    if selected_gate_up_bias is not None:
        selected_gate_up_bias = selected_gate_up_bias[:valid_token_count]
    ...
    if selected_down_bias is not None:
        selected_down_bias = selected_down_bias[:valid_token_count]
    ...
    out_per_sample_g = out_per_sample_g * sample_weights_g[:valid_token_count].unsqueeze(-1)

    # Restore original order
    # out_per_sample = out_per_sample_g[inv_perm]
    # Create a full-sized buffer initialized to zero
    
    full_out_g = torch.zeros(num_tokens * num_top_k, hidden_dim, 
                             dtype=out_per_sample_g.dtype, device=device)
    
    # Fill in the computed results for the valid experts (0-15)
    full_out_g[:valid_token_count] = out_per_sample_g
    
    # Now apply the inverse permutation on the full-sized tensor
    out_per_sample = full_out_g[inv_perm]
